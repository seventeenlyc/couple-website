...
$csrf_token = md5(session_id() . date('Y-m-d'));
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body class="p-4">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-white mb-4">
                ...
                我们的相册
                <i class="fas fa-camera text-pink-500 ml-2"></i>
            </h1>
            <p class="text-xl text-white">
                记录我们的美好时光 📸 共有 <span class="font-bold"><?php echo count($albumData['photos']); ?></span> 张照片
            </p>
        </div>
        ...
        <div class="card">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="console.log('照片数据:', <?php echo json_encode($folderContents['files'], JSON_UNESCAPED_UNICODE); ?>); console.log('原始相册数据:', <?php echo json_encode($albumData['photos'], JSON_UNESCAPED_UNICODE); ?>); alert('查看控制台照片数据')" class="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 transition-colors">
                        <i class="fas fa-bug mr-2"></i>调试照片数据
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div id="multiUploadArea" class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center mb-4 transition-colors hover:border-blue-400 hover:bg-blue-50 cursor-pointer">
                <div id="uploadPrompt">
                    ...
                    <p class="text-sm text-gray-500 mb-4">支持多张照片同时上传 • JPG、PNG、GIF • 最大 5MB/张</p>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-center items-center">
                        ...
                        <button type="button" id="takePhotoBtn" class="bg-pink-500 hover:bg-pink-600 text-white px-6 py-2 rounded-lg transition-colors md:hidden">
        ...
        <div class="card">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php 
                $photoIndex = 0;
                foreach (array_reverse($folderContents['files']) as $photo): 
                ?>
                <div class="bg-gray-50 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group photo-card">
                    <div class="relative">
                        <img src="<?php echo htmlspecialchars($photo['path']); ?>" 
                             ...
                             onclick="handlePhotoClick(<?php echo $photoIndex; ?>)"
                        ...
                        <div class="hidden md:block absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center">
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <button 
                                    onclick="event.stopPropagation(); openPhotoModal('<?php echo htmlspecialchars($photo['id']); ?>')"
                                    class="bg-white text-gray-800 px-4 py-2 rounded-lg mr-2 hover:bg-gray-100 transition-colors"
                                >
                                    <i class="fas fa-eye mr-1"></i>查看
                                </button>
                                <button 
                                    onclick="event.stopPropagation(); deletePhotoByDataId(this)"
                                    class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors"
                                    data-photo-id="<?php echo htmlspecialchars($photo['id']); ?>"
                                    title="删除照片 ID: <?php echo htmlspecialchars($photo['id']); ?>"
                                >
                        ...
                        <!-- 移动端操作菜单 -->
                        <div id="mobileMenu_<?php echo $photoIndex; ?>" class="md:hidden absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center hidden">
                            <div class="flex space-x-4">
                                <button 
                                    onclick="closeMobileMenu(<?php echo $photoIndex; ?>); openPhotoModal('<?php echo htmlspecialchars($photo['id']); ?>')"
                                    class="bg-blue-500 hover:bg-blue-600 text-white rounded-lg px-6 py-3 flex flex-col items-center transition-colors shadow-lg"
                                >
                                    <i class="fas fa-eye text-xl mb-1"></i>
                                    <span class="text-sm">查看</span>
                                </button>
                                <button 
                                    onclick="closeMobileMenu(<?php echo $photoIndex; ?>); deletePhotoByDataId(this)"
                                    class="bg-red-500 hover:bg-red-600 text-white rounded-lg px-6 py-3 flex flex-col items-center transition-colors shadow-lg"
                                    data-photo-id="<?php echo htmlspecialchars($photo['id']); ?>"
                                    title="删除照片 ID: <?php echo htmlspecialchars($photo['id']); ?>"
                                >
        ...
        <div class="bg-white rounded-lg max-w-4xl max-h-full overflow-auto" onclick="event.stopPropagation()">
            ...
            <div class="p-4">
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                    ...
                    <button 
                        onclick="closePhotoModal(); showDeletePhotoConfirm(currentPhotoId, currentPhotoTitle);"
                        class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors"
                    >
    ...
    <script>
        // 照片数据 - 直接使用原始相册数据并过滤当前文件夹
        const allPhotos = <?php echo json_encode($albumData['photos'], JSON_UNESCAPED_UNICODE); ?>;
        const currentFolder = '<?php echo addslashes($currentFolder); ?>';
        const csrfToken = '<?php echo $csrf_token; ?>';
        
        // 过滤当前文件夹的照片并反向排序
        const photoData = allPhotos
            .filter(photo => (photo.folder_path || '') === currentFolder)
            .reverse();
        ...
        function deletePhotoByDataId(button) {
            ...
            console.log('获取到的照片ID:', photoId);
            console.log('照片ID类型:', typeof photoId);
            console.log('照片ID长度:', photoId ? photoId.length : 'undefined');
            ...
            console.log('清理后的照片ID:', cleanPhotoId);
        ...
        async function deletePhoto(photoId) {
            try {
                ...
                const response = await fetch('./temp-photos-api.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
